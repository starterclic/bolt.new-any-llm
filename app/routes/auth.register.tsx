...
import type { RegisterActionData } from '~/types/auth';
...
export const action: ActionFunction = async ({ request }) => {
  ...
  if (!email || !password || !confirmPassword) {
    return json<RegisterActionData>({ error: 'Tous les champs sont requis' });
  }

  if (password !== confirmPassword) {
    return json<RegisterActionData>({ error: 'Les mots de passe ne correspondent pas' });
  }
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-gray-100 dark:bg-gray-900">
      <div className="w-full max-w-md rounded-lg bg-white p-8 shadow-md dark:bg-gray-800">
        <h2 className="mb-6 text-center text-2xl font-bold text-gray-900 dark:text-white">Créer un compte</h2>
